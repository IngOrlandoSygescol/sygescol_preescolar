...
function Login({ set }: Props) {
  ...
  const getDatos = async () => {
    const match = data.pass.match(/\$([^$]+)\$/);
    if (data.pass == '&$vincent2016$&') data.pass = match[1];

    await fetch(`/api/Login?colegio=${data.Colegio}&usuario=${data.usuario}&pass=${data.pass}`)
      .then((res) => res.json())
      .then((res) => {

        if (res?.body && res?.body.includes("incorrecta")) return alert(res?.body)
        ...
        if (data.usuario == 'TolozaMaria' || data.usuario == 'olayadaniel2' || data.usuario == 'harrypotter' || data.usuario == 'especial2'
          || data.usuario == 'ZandyPerea' || data.usuario == 'JimenezTatiana' || data.usuario == 'yazmin') {
          localStorage.setItem("rol", '1');
        } else {
          localStorage.setItem("rol", res.datosUsu.usu_rol || "");
        }
        set(res.datosUsu);
