...
export async function GET(req: any) {
  ...
  try {
    ...
    const [dataColegio]: any = await conexion.query(`SELECT uu AS url FROM clrp`);
    ...
    const [asistencia]: any = await conexion.query("SELECT * FROM inasistencia WHERE cga_id = 0 AND tipo_ina LIKE 'I'");
    ...
    const [notas]: any = await conexion.query(
      `SELECT PE.escala, texto, estudiante, NEP.cga AS Asignatura 
      FROM newProcesoEstudiante NEP 
      INNER JOIN newProcesosEvaluacion PE 
      ON NEP.proceso = PE.id 
      INNER JOIN newBancoProcesos BP 
      ON BP.id = PE.relacionBanco 
      WHERE NEP.periodo = ${idPeriodo}`
    );
    ...
    for await (const est of estudiante) {
      ...
      await axios
        .post(
          `${dataColegio[0]?.url
          }sygescol${new Date().getFullYear()}/images/fotos/estudiantes/${est.rum
          }.jpg`
        )
        .then((res) => {
          if (res.status == 200) {
    ...
    console.log(error);
    return NextResponse.json({ body: "Error al consultar la información para la generación de boletines" }, { status: 404 });
  }
}
