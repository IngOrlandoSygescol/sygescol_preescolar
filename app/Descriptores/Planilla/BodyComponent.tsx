...
function BodyComponent() {
  ...
  const [view, setView] = useState({ open: false, id: 0 } as any);
        ...
        <>
          <div className="flex flex-row justify-between my-2 gap-2 lg:gap-0.5">
            {nacional.map((nac) => {
              return (
                <div
                  className={`rounded-lg cursor-pointer ${escala[row.matricula] == nac?.id && nac?.id < 3
                      ? "bg-green-600"
      ...
      selector: (row: any, key: number) => (
        <>
          <div className="flex flex-wrap gap-2">
            <button
              className={`${estudent.indexOf(row.matricula) >= 0 ? ' bg-blue-600  hover:bg-blue-400' : ' bg-gray-500'} p-2 text-center font-bold rounded-md text-white`}
              disabled={estudent.indexOf(row.matricula) >= 0 ? false : true}
              onClick={() => {
                if (estudent.indexOf(row.matricula) >= 0) openRegistro(row.matricula, escala[row.matricula])
              }}
            >
              Asignar Registro
            </button>
